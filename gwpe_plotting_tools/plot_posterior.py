...
def fillout_pardic(pardic):
    ...
    if ('chi_eff' not in keylist and 'chi_1' in keylist
            and 'chi_2' in keylist and 'mass_ratio' in keylist):
        pardic['chi_eff'] = (
            (pardic['chi_1'] + pardic['mass_ratio'] * pardic['chi_2'])
            / (1. + pardic['mass_ratio'])
        )
    if ('chi_eff' not in keylist and 'a_1' in keylist and 'a_2' in keylist
            and 'mass_ratio' in keylist and 'tilt_1' in keylist
    ...
    if 'symmetric_mass_ratio' not in keylist and 'mass_ratio' in keylist:
        pardic['symmetric_mass_ratio'] = (
            pardic['mass_ratio'] / (1. + pardic['mass_ratio']) ** 2
        )
    if ('total_mass' not in keylist and 'chirp_mass' in keylist
            and 'symmetric_mass_ratio' in keylist):
        pardic['total_mass'] = (
            pardic['chirp_mass'] * pardic['symmetric_mass_ratio'] ** (-0.6)
        )
    if ('chirp_mass' not in keylist and 'total_mass' in keylist
            and 'symmetric_mass_ratio' in keylist):
        pardic['chirp_mass'] = (
            pardic['total_mass'] * pardic['symmetric_mass_ratio'] ** (0.6)
        )
    if ('mass_1' not in keylist and 'total_mass' in keylist
            and 'mass_ratio' in keylist):
        pardic['mass_1'] = pardic['total_mass'] / (1. + pardic['mass_ratio'])
    if ('mass_2' not in keylist and 'total_mass' in keylist
            and 'mass_ratio' in keylist):
        pardic['mass_2'] = (
            pardic['mass_ratio'] * pardic['total_mass']
            / (1. + pardic['mass_ratio'])
        )
    ...
    if 'chi_p' not in keylist:
        if 'tilt_1' in keylist:
            pardic['chi_p'] = gwutils.compute_chi_prec(
                pardic['mass_1'], pardic['mass_2'],
                pardic['a_1'], pardic['a_2'],
                pardic['tilt_1'], pardic['tilt_2']
            )
        elif ('spin_1x' in keylist and 'spin_1y' in keylist
              and 'spin_2x' in keylist and 'spin_2y' in keylist):
            pardic['chi_p'] = gwutils.compute_chi_prec_from_xyz(
                pardic['mass_ratio'],
                pardic['spin_1x'], pardic['spin_1y'],
                pardic['spin_2x'], pardic['spin_2y']
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-i', '--input', type=str, required=True, nargs='*',
        help='Path to hdf5 (bilby)/extrinsic_posterior_samples.dat (rift) file(s).'
    )
    parser.add_argument(
        '-t', '--truth', type=str, default=None, nargs='*',
        help=('Truth file(s) for injections; if not provided, '
              'looking for config with similar name to posterior file.')
    )
    parser.add_argument(
        '-l', '--label', type=str, nargs='*', default=None,
        help='Labels for legends.'
    )
    parser.add_argument(
        '-p', '--par', type=str, default=None, action='append', nargs='*',
        help=('For each corner plot, add an instance specifying what '
              'parameters it should plot. E.g.: plot_posterior.py -i res.hdf5 '
              '-p chirp_mass chi_eff -p eccentricity mean_per_ano')
    )
    parser.add_argument(
        '-r', '--range', type=str, action='append', nargs='*',
        help='Parameter ranges for every par in every corner.'
    )
    parser.add_argument(
        '-k', '--ticks', type=str, action='append', nargs='*', default=None,
        ...
        for jp, parlist in enumerate(args.par):
            ...
            if len(parlist) == 2 and args.triangle:
                ...
                post.make_triangle_plot(
                    ...
                    fill=True,
                    plot_density=True,
                    truth=([truths[par] for par in parlist]
                    ...
                    legend_kwargs={
                        ...
                        'framealpha': 0.75, 'facecolor': 'w',
                        'edgecolor': 'w', 'fontsize': 12
                    }
                )
            else:
                if jp >= len(figs):
                    figs.append(figure(
                        figsize=(args.size * (2 + (len(parlist) - 2)),
                                 args.size * (2 + (len(parlist) - 2))),
                        gca=False
                    ))
                    gs = gridspec.GridSpec(
                        len(parlist), len(parlist),
                        width_ratios=[1] * len(parlist),
                        height_ratios=[1] * len(parlist),
                        wspace=0.0, hspace=0.0
                    )
                    axes = [figs[jp].add_subplot(gs[jjj])
                ...
                ylims_now = [
                    this_ax.get_ylim()[-1]
                    for this_ax in [axes[ii * (len(parlist) + 1)]
                ...
                if args.ticks is not None:
                    for jpar, par in enumerate(parlist):
                        if args.ticks[jp][jpar] is not None:
                            ...
                            # Diagonal
                            axdiag = figs[jp].get_axes()[
                                jpar * len(parlist) + jpar
                            ]
                            axdiag.set_xticks(tickvals)
                            # Off-diagonal x
                            for jrow in range(jpar + 1, len(parlist)):
                                axoffx = figs[jp].get_axes()[
                                    jrow * len(parlist) + jpar
                                ]
                                axoffx.set_xticks(tickvals)
                            # Off-diagonal y
                            for jcol in range(jpar):
                                axoffy = figs[jp].get_axes()[
                                    jpar * len(parlist) + jcol
                                ]
        ...
        for jf, fig in enumerate(figs):
            if not (args.triangle and len(args.par[jf]) == 2):
                fig.legend(
                    ...
                    frameon=False, fontsize=15, loc='upper right'
                )
